...
js_code = """function toggleSection(mode) {
...
function copyToClipboard() {
    ...
    if(input) {
        ...
        copyBtn.innerHTML = '<i class="fas fa-check"></i> ' + window.ShortlinkConfig.translations.copied;
        setTimeout(() => {
...
if (!window.shortlinkEventDelegationSet) {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (btn) {
            ...
            if(form) {
                form.action = window.ShortlinkConfig.routes.updateBase + '/' + card.dataset.id;
            }
            ...
            if(statusPassword) {
                if (card.dataset.password) {
                    statusPassword.innerText = window.ShortlinkConfig.translations.password_protected;
                    if (statusPasswordIcon) statusPasswordIcon.className = 'fas fa-lock';
                } else {
                    statusPassword.innerText = window.ShortlinkConfig.translations.pub_link;
            ...
            if(statusExpires) {
                if (card.dataset.expires) {
                    statusExpires.innerText = window.ShortlinkConfig.translations.expired_label + ': ' + card.dataset.expires.replace('T', ' ');
                } else {
                    statusExpires.innerText = window.ShortlinkConfig.translations.no_time_limit;
                }
            }
            ...
            if(panelDesc) panelDesc.innerText = card.dataset.description || window.ShortlinkConfig.translations.desc_placeholder;
...
with open("public/js/pages/shortlink.js", "w") as f:
    f.write(js_code)

push_scripts = """@push("scripts")
<script>
...
# Find where @push("scripts") starts
start_idx = -1
for i, line in enumerate(lines):
    if '@push("scripts")' in line:
        start_idx = i
        break

if start_idx != -1:
    new_lines = lines[:start_idx] + [push_scripts]
